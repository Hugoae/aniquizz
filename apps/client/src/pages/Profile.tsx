...
export default function Profile() {
  ...
  const [statsData, setStatsData] = useState({
      ...
      discoveredSongs: 0,
      progressPercent: 0,
      rankLabel: "Chargement...",
      rankColor: "text-muted-foreground",
      stats: { gamesPlayed: 0, winRate: 0, accuracy: 0, maxStreak: 0 }
  });
  ...
  useEffect(() => {
      ...
      const onStats = (data: any) => { if (data) setStatsData(data); };
      const onProfileUpdate = () => { 
          toast.success("Profil mis à jour !"); 
          setIsSaving(false); setIsEditingUsername(false); refreshProfile(); 
      };
      const onError = (err: any) => { toast.error(err.message || "Une erreur est survenue"); setIsSaving(false); };
  ...
  const saveUsername = () => { if (!newUsername.trim()) return; socket.emit('update_profile_data', { username: newUsername }); setIsSaving(true); };
    ...
    <>
      ...
      <div className="min-h-screen bg-background pb-20">
        ...
        <main className="pt-24 container max-w-[1400px] mx-auto px-4 space-y-8">
          ...
          <div className="mb-6 flex">
            <Button variant="ghost" size="sm" onClick={() => navigate(-1)} className="gap-2 text-muted-foreground hover:text-primary transition-colors group pl-0 hover:bg-transparent">
                <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
                Retour
            </Button>
          </div>
          
          {/* ✅ FIX: rounded-2xl -> rounded-xl (Layer 1) */}
          <div className="relative rounded-xl overflow-hidden bg-card border border-white/10 p-8 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
              ...
              <div className="flex-1 space-y-2 py-2 w-full">
                <div className="flex flex-col md:flex-row items-center md:items-end gap-3 min-h-[48px]">
                    {isEditingUsername ? (
                        ...
                        <div className="flex items-center gap-3 group">
                            ...
                            {/* ✅ FIX: Suppression de rounded-full pour hériter du rounded-md (standard) */}
                            <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-primary" onClick={() => setIsEditingUsername(true)}><Edit2 className="h-5 w-5" /></Button>
                        </div>
                    )}
                    {/* ✅ FIX: rounded -> rounded-sm */}
                    {profile.role === 'ADMIN' && <span className="text-xs bg-red-500/20 text-red-400 px-2 py-1 rounded-sm border border-red-500/50 mb-2 md:mb-1">ADMIN</span>}
                </div>
                ...
                {profile.anilistUsername && (
                    /* ✅ FIX: rounded-full -> rounded-md */
                    <div className="flex items-center justify-center md:justify-start gap-2 text-sm text-blue-400 mt-2 bg-blue-500/10 px-3 py-1 rounded-md w-fit mx-auto md:mx-0">
          ...
          <div className="grid grid-cols-12 gap-8">
              <div className="col-span-12 lg:col-span-9 space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      <StatCard icon={Trophy} label="Taux de victoire" value={`${statsData.stats.winRate}%`} color="text-yellow-500" />
                      <StatCard icon={Check} label="Taux de bon guess" value={`${statsData.stats.accuracy}%`} color="text-green-500" />
                      <StatCard icon={Target} label="Parties jouées" value={statsData.stats.gamesPlayed} color="text-blue-400" />
                      <StatCard icon={Flame} label="Best Streak" value={statsData.stats.maxStreak} color="text-orange-500" />
                  </div>
                  ...
                  <div className="pt-4">
                      ...
                      <div className="bg-card border border-white/10 rounded-xl p-6 shadow-lg relative overflow-hidden">
                          <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-4 z-10 relative">
                              <div>
                                  <div className="text-4xl font-black gradient-text">{statsData.discoveredSongs}</div>
                                  <div className="text-sm text-muted-foreground font-medium">Sons uniques découverts</div>
                              </div>
                              <div className="text-right">
                                  {/* ✅ FIX: rounded-full -> rounded-md */}
                                  <span className={`px-3 py-1 bg-white/5 rounded-md text-xs font-bold border border-white/10 ${statsData.rankColor}`}>
                                      {statsData.rankLabel}
                                  </span>
                              </div>
                          </div>
                          ...
                          <div className="space-y-2 z-10 relative">
                              ...
                              <div className="h-4 bg-secondary/50 rounded-lg overflow-hidden border border-white/5 relative">
                                  <div 
                                    ...
                                    style={{ 
                                        width: `${Math.min(statsData.progressPercent, 100)}%`,
                              ...
                              <div className="flex justify-between text-xs font-mono text-muted-foreground">
                                  <span>0</span>
                                  <span>Total Disponible: {statsData.totalSongs}</span>
                              </div>
                          </div>
      ...
      <Dialog open={showCropModal} onOpenChange={setShowCropModal}>
        <DialogContent className="sm:max-w-md bg-card border-border">
          ...
          <div className="relative w-full h-64 bg-black rounded-lg mt-4">{selectedFile && (<Cropper image={selectedFile} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={(area, pixels) => setCroppedAreaPixels(pixels)} />)}</div>
      ...
      <Dialog open={showAniListModal} onOpenChange={setShowAniListModal}>
        <DialogContent className="sm:max-w-md bg-card border-border">
            ...
            <div className="py-4"><Input placeholder="Votre pseudo AniList exact..." value={anilistName} onChange={(e) => setAnilistName(e.target.value)} /></div>
            <DialogFooter><Button onClick={handleLinkAnilist} disabled={isSyncing}>{isSyncing ? <Loader2 className="animate-spin h-4 w-4" /> : "Sauvegarder"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  );
