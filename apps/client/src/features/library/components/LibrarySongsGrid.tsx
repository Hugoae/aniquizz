...
export function LibrarySongsGrid({ songs, onSelectSong }: LibrarySongsGridProps) {
  ...
  return (
    <VirtualScroll
      ...
      threshold={12}
      remeasureKey={playingId}
      listLabel={LIBRARY_COPY.viewSongs}
      getKey={(song) => song.id}
      renderItem={(song) => (
        <LibrarySongCard
          song={song}
          isInlinePlaying={playingId === song.id}
          onTogglePreview={() => toggle(song.id)}
          onOpen={() => onSelectSong(song, stopAndCapture(song.id))}
          onStopPreview={stop}
          videoRef={videoRef}
        />
      )}
...
function LibrarySongCard({
  ...
  return (
    <article className="glass-card mb-2 overflow-hidden transition-colors hover:border-primary/40">
      <div className="group flex w-full items-center gap-3 p-3">
        <button
          type="button"
          onClick={onTogglePreview}
          aria-label={isInlinePlaying ? LIBRARY_COPY.previewPause : LIBRARY_COPY.playPreview}
          className={cn(
