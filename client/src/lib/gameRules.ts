...
export const getRank = (score: number, maxScore: number) => {
    if (maxScore === 0) return GAME_RULES.RANKS[GAME_RULES.RANKS.length - 1];
    const ratio = score / maxScore;
    return GAME_RULES.RANKS.find(r => ratio >= r.threshold) || GAME_RULES.RANKS[GAME_RULES.RANKS.length - 1];
};