...
export default function Profile() {
  ...
  useEffect(() => {
      ...
      const onStats = (data: any) => { 
          if (data && data.songs) setTotalSongsCount(data.songs); 
      };
  ...
  const discoveredSongs = extProfile.history && Array.isArray(extProfile.history) && extProfile.history.length > 0 ? (extProfile.history[0] as any).count : 0;
  const gamesPlayed = profile.gamesPlayed || 0;
  const gamesWon = profile.gamesWon || 0;
  const winRate = gamesPlayed > 0 ? Math.round((gamesWon / gamesPlayed) * 100) : 0;
  const totalGuesses = extProfile.totalGuesses || 0;
  const correctGuesses = extProfile.correctGuesses || 0;
  const guessAccuracy = totalGuesses > 0 ? Math.round((correctGuesses / totalGuesses) * 100) : 0;
  const maxStreak = extProfile.maxStreak || 0;
        ...
        <main className="pt-24 container max-w-[1400px] mx-auto px-4 space-y-8">
          ...
          <div className="mb-6 flex">
            <Button 
                ...
                size="sm" 
                onClick={() => navigate(-1)} 
                className="gap-2 text-muted-foreground hover:text-primary transition-colors group pl-0 hover:bg-transparent"
            >
          ...
          <div className="grid grid-cols-12 gap-8">
              ...
              <div className="col-span-12 lg:col-span-9 space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <StatCard icon={Target} label="Parties jouées" value={gamesPlayed} color="text-blue-400" />
                  ...
                  <div className="pt-4">
                      ...
                      <div className="bg-card border border-white/10 rounded-2xl p-6 shadow-lg relative overflow-hidden">
                          ...
                          <div className="space-y-2 z-10 relative">
                              <div className="h-4 bg-secondary/50 rounded-full overflow-hidden border border-white/5 relative">
                                  {/* CORRECTION BARRE PROGRESSION */}
                                  <div 
                                    className="h-full bg-gradient-to-r from-purple-600 to-pink-500 transition-all duration-1000 ease-out" 
                                    style={{ 
                                        width: `${Math.min(pokedexProgress, 100)}%`,
