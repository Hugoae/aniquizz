...
| **AniList**               | GraphQL `MediaListCollection` → internal `Anime.id`. Statuses: Completed, Watching, On-Hold, Rewatching, Dropped. Planning stays out.                                                                                                                                                 |
| **MAL**                   | Official v2 `GET /users/{name}/animelist` with `X-MAL-CLIENT-ID` (no OAuth); a profile-page HEAD disambiguates missing users from private-list 404s. Statuses: `watching`, `completed`, `on_hold`, `dropped` → catalogue via `Anime.idMal`. Planning / plan-to-watch stays out.       |
...
Shared helpers: `packages/shared/src/watchedList.ts` (`hasWatchedListLink`, `resolveActiveListProvider`) and `packages/shared/src/watchedStatuses.ts` (which list statuses count as Watched). Socket payloads expose both usernames plus `activeListProvider` on `GamePlayer` / `SocketData`.
