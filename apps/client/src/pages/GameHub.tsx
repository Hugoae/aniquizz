...
import { User, Users, Swords, ArrowLeft, Plus, Lock } from 'lucide-react';
...
import { GameConfig, RoomConfig, GameMode, GAME_CONFIG } from '@aniquizz/shared'; 
...
const modeCards = [ 
    ...
    { id: 'competitive' as GameMode, title: 'Compétitif', description: 'Mode classé avec rangs et saisons. (Bientôt disponible)', icon: Swords, gradient: 'from-slate-500 to-slate-700', disabled: true }, 
];
...
export default function GameHub() {
  ...
  // Helper pour la gestion des types de sons dans le formulaire (UI Only)
  const handleToggleSoundType = (setter: React.Dispatch<React.SetStateAction<any>>) => (type: string) => {
    setter((prev: any) => {
        const current = prev.soundTypes || [];
        if (current.includes(type)) {
            if (current.length <= 1) return prev; // Au moins un type sélectionné
            return { ...prev, soundTypes: current.filter((t: string) => t !== type) };
        }
          ...
          <DialogContent className="sm:max-w-3xl bg-card border-border" onOpenAutoFocus={(e) => e.preventDefault()}>
            <DialogHeader>
                <DialogTitle className="text-xl flex items-center gap-3">
                    {roomConfig.maxPlayers === 1 ? "Paramètres de la partie" : "Paramètres du salon"}
                </DialogTitle>
                <DialogDescription>Configurez les règles.</DialogDescription>
            </DialogHeader>
            <GameConfigForm 
                config={roomConfig} 
                setConfig={setRoomConfig as any} 
                toggleSoundType={handleToggleSoundType(setRoomConfig)}
                onReset={() => setRoomConfig(defaultRoomConfig)}
                onSubmit={handleCreateOrUpdateRoom} 
                isRoom={true} 
                hideRoomSettings={roomConfig.maxPlayers === 1}
                playlists={GAME_CONFIG.PLAYLISTS} 
              ...
              <div className="relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
                  {modeCards.map((mode, index) => ( 
                    <button key={mode.id} onClick={() => handleModeSelect(mode.id)} disabled={mode.disabled} className={cn("glass-card p-10 text-left hover-lift hover-glow group cursor-pointer animate-fade-in transition-all relative overflow-hidden flex flex-col justify-center min-h-[300px]", mode.disabled && "opacity-60 cursor-not-allowed grayscale-[0.3]")} style={{ animationDelay: `${index * 0.1}s` }} > 
                      <div className={`inline-flex p-5 rounded-lg bg-gradient-to-br ${mode.gradient} mb-6 w-fit`}><mode.icon className="h-8 w-8 text-white" /></div> 
        ...
        <Dialog open={showConfig} onOpenChange={setShowConfig}>
            <DialogContent className="sm:max-w-3xl bg-card border-border" onOpenAutoFocus={(e) => e.preventDefault()}>
                ...
                <GameConfigForm 
                    ...
                    toggleSoundType={handleToggleSoundType(setConfig)}
