    ...
    <div className="flex flex-col h-full max-h-[80vh]">
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6 p-1">
        ...
        {isRoom && !hideRoomSettings && (
            <div className="space-y-4 bg-secondary/10 p-4 rounded-xl border border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            <div className="space-y-6">
                ...
                {config.responseType !== 'qcm' && (
                    <div className="space-y-3">
                        <SectionHeader icon={Target} title="Précision" tooltip="Degré de précision requis." />
                ...
                <div className="bg-secondary/10 border border-white/5 p-4 rounded-xl space-y-5 shadow-inner">
                    ...
                    {isDecadeSelected && (
                        <div className="space-y-2 animate-in slide-in-from-top-2 pt-2 border-t border-white/5">
                            <Label className="text-xs font-bold text-purple-400 uppercase flex items-center gap-2"><Calendar className="w-3 h-3" /> Décennie</Label>
                            <div className="grid grid-cols-3 gap-2">
                                {decades.map(decade => (
                                    <button key={decade} onClick={() => setConfig((prev: any) => ({ ...prev, decade }))} className={cn("py-1.5 rounded-md text-[10px] font-bold border transition-all", config.decade === decade ? "bg-purple-500 text-white border-purple-400 shadow-lg shadow-purple-500/20" : "bg-card border-white/10 hover:border-white/30 text-muted-foreground")}>{decade}s</button>
                                ))}
                            </div>
                        </div>
                    )}
            ...
            {/* DROITE : Source des Sons */}
            <div className="space-y-4">
                <SectionHeader 
                    icon={Eye} 
                    title="Source des musiques" 
                    tooltip="D'où proviennent les animes ? Pour utiliser 'Watched', vous devez être connecté et avoir lié votre compte AniList." 
                />
                <div className="flex bg-secondary/20 p-1 rounded-lg">
                    ...
                    {/* ✅ Bouton Watched : Grisé si !user */}
                    <button 
                        onClick={() => !isWatchedDisabled && handleSourceChange('watched')} 
                        ...
                        className={cn(
                            ...
                            (config.soundSelection as string) === 'watched' ? "bg-background shadow text-primary" : "text-muted-foreground hover:text-foreground",
                            isWatchedDisabled && "opacity-50 cursor-not-allowed hover:text-muted-foreground"
                        )}
                    >
                        Watched {isWatchedDisabled && <Lock className="w-3 h-3" />}
                    </button>
                ...
                <div className="min-h-[250px] bg-card/30 rounded-xl border border-white/5 p-2">
                    {(config.soundSelection as string) === 'random' && (
                        <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-center p-4 text-muted-foreground animate-in fade-in zoom-in duration-300">
                            <div className="bg-primary/10 p-4 rounded-full mb-3"><Shuffle className="w-8 h-8 text-primary" /></div>
                        ...
                        <div className="space-y-4 animate-in fade-in zoom-in duration-300">
                            <div className="grid grid-cols-1 gap-2">
                                {playlists.map(playlist => {
                                    ...
                                    return ( 
                                        <button key={playlist.id} onClick={() => setConfig((prev: any) => ({ ...prev, playlist: playlist.id, decade: playlist.id === 'decades' ? '2010' : null }))} className={cn("flex items-center gap-3 p-2.5 rounded-lg border text-left transition-all hover:border-primary/50", isSelected ? "border-primary bg-primary/10" : "border-border bg-card hover:bg-secondary/50")} style={{ borderColor: isSelected ? playlist.color : undefined, backgroundColor: isSelected ? `${playlist.color}15` : undefined }}> 
                                            <div className="flex items-center gap-2"><span className="text-lg" style={{ color: playlist.color}}>{playlist.icon}</span><div className="flex-1 min-w-0"><div className="text-xs font-medium truncate">{playlist.name}</div><div className="text-[10px] text-muted-foreground">{playlist.count}</div></div></div> 
                                        </button> 
                                    );
                                })}
                            </div>
      ...
      <div className="flex gap-3 pt-4 mt-2 border-t border-white/5 shrink-0 bg-background/50 backdrop-blur-sm">
          <Button variant="outline" onClick={onReset} className="gap-2 rounded-lg"><RotateCcw className="h-4 w-4" /> Reset</Button>
          <Button variant="glow" onClick={onSubmit} className="flex-1 text-lg font-bold rounded-lg" disabled={config.soundTypes.length === 0 || (isRoom && !hideRoomSettings && (!roomConfig.roomName || (roomConfig.isPrivate && !roomConfig.password)))}>
