...
describe('isTrustedSupabaseAvatarUrl', () => {
  ...
  it('rejects another host, bucket, user, or scheme', () => {
    expect(
      isTrustedSupabaseAvatarUrl(owned.replace(ORIGIN, 'https://evil.example'), ORIGIN, USER),
    ).toBe(false);
    expect(isTrustedSupabaseAvatarUrl(owned.replace('/avatars/', '/other/'), ORIGIN, USER)).toBe(
      false,
    );
    expect(
    ...
    expect(isTrustedSupabaseAvatarUrl(owned.replace('https://', 'http://'), ORIGIN, USER)).toBe(
      false,
    );
  });
  ...
  it('rejects credentials, hashes, and path tricks', () => {
    expect(
      isTrustedSupabaseAvatarUrl(
        `https://user:pass@abcdxyz.supabase.co/storage/v1/object/public/avatars/${USER}/avatar.jpg`,
        ORIGIN,
        USER,
      ),
    ).toBe(false);
    ...
    expect(
      isTrustedSupabaseAvatarUrl(
        `${ORIGIN}/storage/v1/object/public/avatars/${USER}/../avatar.jpg`,
        ORIGIN,
        USER,
      ),
    ).toBe(false);
